...
try {
    ...
    if (!isset($data->id) || !isset($data->user_id) || !isset($data->quantity)) {
        echo json_encode(["status" => "error", "message" => "Missing required data"]);
        exit;
    }
    
    // ตรวจสอบว่าเป็น cart item ของ user นี้หรือไม่ (ป้องกันการแก้ไขข้อมูลของ user อื่น)
    ...
    $update_stmt->execute([$data->quantity, $data->id, $data->user_id]);
